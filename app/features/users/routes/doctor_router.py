...
from app.features.users.schemas.receptionist_schema import ReceptionistCreate, ReceptionistResponse
from typing import List

router = APIRouter(prefix="/doctors", tags=["Doctors"])

@router.post("/{doctor_id}/receptionists", response_model=ReceptionistResponse, status_code=status.HTTP_201_CREATED)
def create_receptionist(doctor_id: int, data: ReceptionistCreate, db: Session = Depends(get_db)):
    try:
        return doctor_service.create_receptionist(db=db, doctor_id=doctor_id, data=data)
...
def get_receptionists_by_doctor(doctor_id: int, db: Session = Depends(get_db)):
    try:
        return doctor_service.get_receptionists_by_doctor(db=db, doctor_id=doctor_id)
    except ValueError as e:
